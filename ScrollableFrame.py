...
class ScrollableFrame:
    def __init__(self, master, frame_fg_color, frame_expand, frame_fill, frame_padx, frame_pady):
        ...
        self.scrollable_frame = CTkScrollableFrame(master=self.master, fg_color=self.frame_fg_color)
        self.scrollable_frame.pack(expand=self.frame_expand, fill=self.frame_fill, padx=self.frame_padx, pady=self.frame_pady)
    ...
    def create_table(self, values, colors, header_color, hover_color, edit_row=None, edit_text_color=None, edit_hover_color=None, table_expand=None):
        if self.validate_table_data(values):
            ...
            table.edit_row(edit_row, text_color=edit_text_color, hover_color=edit_hover_color)
    ...
    def lable(self,label_text,font_family,font_size,text_color, bg_color,row,col,ancher,expand,padx=None,pady=None):
        ...
        label = CTkLabel(master=self.scrollable_frame, text=f"{label_text}",font=(f"{font_family}", font_size), text_color=text_color, bg_color=bg_color)
...
table_frame = ScrollableFrame(master=app, frame_fg_color="transparent", frame_expand=True, frame_fill="both", frame_padx=27, frame_pady=21)

table_frame.create_table(values=table_data, colors=["#E6E6E6", "#EEEEEE"], header_color="#2A8C55", hover_color="#B4B4B4",edit_row=0,edit_text_color="#fff",edit_hover_color="#2A8C55",table_expand=True)
